...
from forms import CreatePostForm, RegisterForm, LoginForm, CommentForm, SubjectiveQuestion
...
def register():
    ...
    if form.validate_on_submit():
        ...
        result_quest = db.session.execute(db.select(User).where(User.question == form.question.data))
        ...
        hash_and_salted_answer = generate_password_hash(
            form.answer.data,
            method='pbkdf2:sha256',
            salt_length=16
        )
...
def ask_ans_User():
    ...
    return render_template("subjective.html", form=form, current_user=current_user)
